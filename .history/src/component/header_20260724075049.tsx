import Link from "next/link";

export default function Header() {
    return (
        <div>
            <div className="mx-auto flex max-w-360 items-center justify-between px-6 py-4 lg:px-10">
                <a href="index.html" className="flex items-center gap-4">
                    <Link src="assets/img/logo-weg.png" alt="Logo WEG" className="h-9 w-auto"><Link></Link>
                        <span className="h-8 w-px bg-zinc-300"></span>
                        <span className="text-sm font-bold uppercase tracking-[0.22em] text-zinc-700">Warehouse</span>
                </a>
                <nav className="flex items-center gap-7 text-sm font-semibold">
                    <a href="index.html" className="text-blue-800">Painel</a>
                    <a href="sobre.html" className="text-zinc-600 hover:text-blue-800">Sobre o sistema</a>
                </nav>
            </div>
        </div>
    )
}