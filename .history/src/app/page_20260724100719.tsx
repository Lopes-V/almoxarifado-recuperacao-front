...
function classificacao(ocupacao: number) {
    if (ocupacao >= 0 && ocupacao <= 20) { const className = `<div className="h-full w-[${ocupacao}%] bg-emerald-500"></div>`; return <div className="mt-5 h-3 bg-zinc-100">{className}</div> }
    if (ocupacao > 20 && ocupacao <= 40) { const className = `<div className="h-full w-[${ocupacao}%] bg-green-500"></div>`; return <div className="mt-5 h-3 bg-zinc-100">{className}</div> }
    if (ocupacao > 40 && ocupacao <= 55) { const className = `<div className="h-full w-[${ocupacao}%] bg-lime-500"></div>`; return <div className="mt-5 h-3 bg-zinc-100"></div> }
    if (ocupacao > 55 && ocupacao <= 65) { const className = `<div className="h-full w-[${ocupacao}%] bg-amber-400"></div>`; return <div className="mt-5 h-3 bg-zinc-100">{className}</div> }
    if (ocupacao > 65 && ocupacao <= 80) { const className = `<div className="h-full w-[${ocupacao}%] bg-orange-500"></div>`; return <div className="mt-5 h-3 bg-zinc-100">{className}</div> }
    ...
    if (ocupacao > 90) { const className = `<div className="h-full w-[${ocupacao}%] bg-rose-800"></div>`; return <div className="mt-5 h-3 bg-zinc-100">{className}</div> }
}
function filtrarNivel(data:posicao[],nivel: string) {

  let html : string= ""
  data.map(e => if(e.corredor === "Corredor A"){
    
    html += `  <a href="posicao.html" className="border-b border-r border-white bg-amber-400 p-5 text-center text-white hover:brightness-95">
  ...
  html += data.map(e => e.corredor === "Corredor B")
  html += data.map(e => e.corredor === "Corredor C")
  html += data.map(e => e.corredor === "Corredor D")
  return html;
}
